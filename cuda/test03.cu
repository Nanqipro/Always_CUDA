...
// 定义全局常量EPSILON，用于比较浮点数时判断是否足够接近
const double EPSILON = 1.0e-15;

// 定义全局常量a、b、c，用于后续的数学计算或比较
const double a = 1.23;
const double b = 2.34;
const double c = 3.57;

// __global__函数add1用于将两个数组的元素逐个相加，结果存储在第三个数组中
// 参数x, y: 输入数组，分别存储要相加的两个数组
// 参数z: 输出数组，存储x和y对应元素相加的结果
// 参数N: 指定数组的长度
void __global__ add1(const double *x, const double *y, double *z, const int N);

// __global__函数add2用于将两个数组的元素逐个相加并乘以一个常数，结果存储在第三个数组中
// 参数x, y: 输入数组，分别存储要相加的两个数组
// 参数z: 输出数组，存储x和y对应元素相加并乘以常数后的结果
// 参数N: 指定数组的长度
void __global__ add2(const double *x, const double *y, double *z, const int N);

// __global__函数add3用于将两个数组的元素逐个相加并进行某种数学运算，结果存储在第三个数组中
// 参数x, y: 输入数组，分别存储要相加的两个数组
// 参数z: 输出数组，存储x和y对应元素相加并进行数学运算后的结果
// 参数N: 指定数组的长度
void __global__ add3(const double *x, const double *y, double *z, const int N);

// 函数check用于检查计算结果是否符合预期
// 参数z: 计算得到的结果数组
// 参数N: 指定数组的长度
